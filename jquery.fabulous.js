!function($) {
  ...
  function d3_interpolateCubehelix(γ) {
    return function(a, b) {
      a = d3.hsl(a);
      b = d3.hsl(b);
      var ah = (a.h + 120) * d3_radians, bh = (b.h + 120) * d3_radians - ah, as = a.s, bs = b.s - as, al = a.l, bl = b.l - al;
      if (isNaN(bs)) bs = 0, as = isNaN(as) ? b.s : as;
      if (isNaN(bh)) bh = 0, ah = isNaN(ah) ? b.h : ah;
      return function(t) {
        var h = ah + bh * t, l = Math.pow(al + bl * t, γ), a = (as + bs * t) * l * (1 - l);
        return "#" + hex(l + a * (-.14861 * Math.cos(h) + 1.78277 * Math.sin(h))) + hex(l + a * (-.29227 * Math.cos(h) - .90649 * Math.sin(h))) + hex(l + a * +1.97294 * Math.cos(h));
      };
    };
  }
  function hex(v) {
  ...
  $.fn.fabulous = function(options) {
    ...
    period = 6;
    var a = +new Date();
    var styleTagId = "fabulous-styles", classPrefix = "fabulous-selection-", style = $("<style>").attr("id", styleTagId).appendTo("body"), styles = [], scale = function() {
      var uninterp = uninterpolate(0, period), interp = interpolate(0, 360);
      return function(x) {
        return interp(uninterp(x));
      };
    }();
    var hcl = function(index) {
    ...
    Array.apply(null, Array(period)).map(function(_, i) {
      return i;
    }).forEach(function(i) {
